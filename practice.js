...
// REPLACE THIS WITH YOUR CODE
const objLit = {students: 24, mentors: 3, instructors: 5}

const {students, mentors, instructors} = objLit
...
// REPLACE THIS WITH YOUR CODE
const greeting = ({name, title}) => `Hello, ${title} ${name}!`
...
// REPLACE THIS WITH YOUR CODE
const gemInfo = (gemType, gemSize, gemWeight) => ({gemType: gemType, gemSize: gemSize, gemWeight: gemWeight});
...
// REPLACE THIS WITH YOUR CODE
const identifier = (jobs) => jobs.filter((job) => job.programmer)[0];
...
// REPLACE THIS WITH YOUR CODE
const startWithLetterA = (arr) => arr.filter((el) => el.startsWith("A") || el.startsWith("a"));
...
// REPLACE THIS WITH YOUR CODE
const formalGreeting = (arr) => arr.map((el, i) => arr[i] = `Hello, ${el}`);
...
// REPLACE THIS WITH YOUR CODE
const productOfArray = (nums) => nums.reduce((acc, curr) => acc * curr);
